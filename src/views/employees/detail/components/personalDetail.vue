  ...
  methods: {
    async loadUserInfo() {
      ...
      if (this.userInfo.staffPhoto !== ' ') {
        this.$refs.imgUpload1.fileList.push({
    ...
    async loadUserDetail() {
      ...
      if (this.formData.staffPhoto !== ' ') {
        this.$refs.imgUpload2.fileList.push({
    ...
    async saveUserInfo() {
      ...
      if (fileList.some(item => item.loadStatus !== true)) { // judge image whether upload successful
        this.$message({ type: 'warning', message: '图片还未上传成功' })
        return
      }
      try {
